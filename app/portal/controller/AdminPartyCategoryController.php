...
class AdminPartyCategoryController extends AdminBaseController
{
    /**
     * 文章分类列表.
     *
    ...
    public function index()
    {
        ...
        if (empty($keyword)) {
            $categoryTree = $portalCategoryModel->adminCategoryTableTree(0, '', 3);
            $this->assign('category_tree', $categoryTree);
        } else {
            $categories = $portalCategoryModel->alias('a')
                ->join('cmf_city b', 'a.city_id = b.id')
                ->where('a.division_id', 'eq', "3")
    ...
    public function add()
    {
        ...
        $portalCategoryModel = new PortalCategoryModel();
        $categoriesTree = $portalCategoryModel->adminCategoryTree($parentId, 0, 3);
    ...
    public function addPost()
    {
        ...
        $this->success('添加成功!', url('AdminPartyCategory/index?division_id=3'));
    }
    ...
    public function edit()
    {
        ...
        if ($id > 0) {
            ...
            $portalCategoryModel = new PortalCategoryModel();
            $categoriesTree = $portalCategoryModel->adminCategoryTree($category['parent_id'], $id, 3);
